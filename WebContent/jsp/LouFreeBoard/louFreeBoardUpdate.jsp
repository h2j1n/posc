...
<div>
    ...
    <form action="/UpdateBoard.do" method="POST" id="updateBoard">
        <input type="hidden" name="freeNo" value="<%=boardVO.getFree_no() %>" /> 
        <input type="hidden" name="writer" value="<%=boardVO.getUser_no() %>" /> 
        <div class = formtitle>
            <label for="title"></label><br>
            <input type="text" id="title" name="title" value="<%=boardVO.getFree_title() %>" placeholder="제목을 입력해주세요" required><br>
        </div>
        <label for="content" style="font-size: 20px;"></label>
        <textarea class="form-control summernote" rows="5" id="content" name="content"><%=boardVO.getFree_content() %></textarea>
    </form>
    </div>>
</div>
