...
public class AdInsert extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        request.getRequestDispatcher("/jsp/ad/adForm.jsp").forward(request, response);
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        // 서비스를 통해 광고 등록
        int status = service.insertAd(adList);
        ...
        if (status > 0) {
         ...
         // 세션 객체 가져오기
            ...
            // userId가 null인지 확인하여 로그인 여부를 판단할 수 있음
            if (userId != null) {
            	response.sendRedirect("/loginMain.do");
            } else {
                // 로그인되지 않은 사용자의 작업 수행
            	response.sendRedirect("/unLoginMain.do");
            }
