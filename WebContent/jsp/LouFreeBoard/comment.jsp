...
<div class="comment_container">
    ...
    <%
        if (!comments.isEmpty()) {
            for (FrcoVO comment : comments) {
    %>
    <div class="comment">
        <div class="comment_header">
            <span class="author"><%= comment.getUser_id() %></span>
            <span class="date"><%= comment.getFrco_date() %></span>
        </div>
        <div class="comment_body">
            <span class="comment_content"><%= comment.getFrco_cont() %></span>
            ...
            <span class="edit_buttons">
                ...
                <input type="hidden" class="comment_no" value="<%= comment.getFrco_no() %>" data-frco-no="<%= comment.getFrco_no() %>">
                <button class="delcombtn" data-frco-no="<%= comment.getFrco_no() %>">
    ...
    <form class="comment_form" method="POST" id="insertcomment">
        ...
        <input type="hidden" id="freeNo" value="<%= request.getParameter("freeNo") %>">
...
<script>
    $(document).on("click", ".updatecombtn", function() {
        ...
        var freeNo = <%=lou.getFree_no() %>; // 해당 댓글의 게시글 번호 가져오기
        
        console.log("freeNo:", freeNo); // freeNo 값 확인
        
        // 기존 댓글 내용을 입력 폼으로 변경
        var updateForm = '<form class="update_comment_form" method="POST" action="/LoutFreeBoard/commentupdate.do">' +
                            ...
                            '<input type="hidden" name="freeNo" value="' + freeNo + '">' + // 게시글 번호 추가
...
<script>
    $(function() {
        ...
        addBtn.on("click", function() {
            ...
            // 폼에 no 값이 있는지 확인
            if (<%= request.getParameter("freeNo") %> == "") {
        ...
        $(document).on("click", ".delcombtn", function() {
            ...
            if (confirm("댓글을 삭제하시겠습니까?")) {
                ...
                var frco_no = $(this).closest('.comment').find('.comment_no').val();
                ...
                // 확인을 누르면 삭제 진행
                location.href = "/LoutFreeBoard/commentdelete.do?cono=" + frco_no + "&freeNo=" + freeNo;
            }
        });
    });
</script>
