...
<%
    PortfolioVO portVO = (PortfolioVO) request.getAttribute("portVO");
%>
...
<div>
    ...
    <form action="/portModify.do" method="POST" id="updateBoard">
        <input type="hidden" name="portNo" value="<%=portVO.getPort_no() %>" /> 
        <div class = formtitle>
            <label for="title"></label><br>
            <input type="text" id="title" name="title" value="<%=portVO.getPort_title() %>" placeholder="제목을 입력해주세요" required><br>
        </div>
        <label for="content" style="font-size: 20px;"></label>
        <textarea class="form-control summernote" rows="5" id="content" name="content"><%=portVO.getPort_content() %></textarea>
    </form>
</div>
